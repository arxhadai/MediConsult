...
class AiRepositoryImpl implements AiRepository {
  ...
  @override
  Future<Either<Failure, SymptomAnalysis>> analyzeSymptoms(String symptoms) async {
  ...
  @override
  Future<Either<Failure, ChatMessage>> sendChatMessage(ChatMessage message) async {
    ...
    final result = await _geminiAiService.sendChatMessage(messageModel, emptyHistory);
    return result.fold(
      (failure) => Left(failure),
      (model) => Right(model.toEntity()),
    );
  ...
  Future<Either<Failure, ConsultationSummary>> generateSummary(
    ...
    final result = await _geminiAiService.generateSummary(consultationTranscript);
    return result.fold(
      (failure) => Left(failure),
      (model) => Right(model.toEntity()),
    );
